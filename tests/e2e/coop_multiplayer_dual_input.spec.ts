...
test.describe('Milestone M35: Local 2-Player Co-op Dual-Input E2E Matrix Suite', () => {
  ...
  test('TC-M35-COOP-01: Concurrent PC dual keyboard input operates without stall across 600 frames', async ({ page }) => {
    ...
    // 1. Enter 2-Player Co-op Mode and transition to active PLAYING state
    await page.evaluate(() => {
    ...
    // Simulate concurrent continuous gameplay loop
    ...
    // Fire an active concurrent volley before releasing movement
    ...
    const finalCoords = await page.evaluate(() => {
      ...
      const p1Shots = game.scoreManager ? game.scoreManager.getShotsFired('p1') : 0;
      const p2Shots = game.scoreManager ? game.scoreManager.getShotsFired('p2') : 0;
    ...
    // Bullets spawned from both players (either currently active or recorded fired)
    expect(finalCoords.totalBullets + finalCoords.totalShots).toBeGreaterThanOrEqual(2);
  ...
  test('TC-M35-COOP-02: Concurrent mobile multi-touch split-screen drives both players without touch collision', async ({ page }) => {
    ...
    // Initialize co-op mode and skip intro to active PLAYING state
    await page.evaluate(() => {
      const cheat = (window as any).__GALAGA_CHEAT__;
      const game = cheat.getGame();
      game.setCoopMode(true);
    ...
    await page.evaluate(
      ({ p1X, p2X, p1Fire, p2Fire, y }) => {
        ...
        const t1 = makeTouch(canvasEl, 1, p1X, y);
        const t2 = makeTouch(canvasEl, 2, p1Fire, y);
        const t3 = makeTouch(canvasEl, 3, p2X, y);
        const t4 = makeTouch(canvasEl, 4, p2Fire, y);

        dispatchTouches(canvasEl, 'touchstart', [t1, t2, t3, t4], [t1, t2, t3, t4]);
      },
    ...
    for (let step = 1; step <= steps; step++) {
      ...
      await page.evaluate(
        ({ cur1, cur2, p1Fire, p2Fire, y }) => {
          ...
          const t1 = makeTouch(canvasEl, 1, cur1, y);
          const t2 = makeTouch(canvasEl, 2, p1Fire, y);
          const t3 = makeTouch(canvasEl, 3, cur2, y);
          const t4 = makeTouch(canvasEl, 4, p2Fire, y);

          dispatchTouches(canvasEl, 'touchmove', [t1, t2, t3, t4], [t1, t3]);
        },
    ...
    await page.evaluate(
      ({ p1EndX, p2EndX, p1Fire, p2Fire, y }) => {
        ...
        const t1 = makeTouch(canvasEl, 1, p1EndX, y);
        const t2 = makeTouch(canvasEl, 2, p1Fire, y);
        const t3 = makeTouch(canvasEl, 3, p2EndX, y);
        const t4 = makeTouch(canvasEl, 4, p2Fire, y);

        dispatchTouches(canvasEl, 'touchend', [], [t1, t2, t3, t4]);
      },
  ...
  test('TC-M35-COOP-03: Symmetrical 3-zone bottom dashboard renders independent P1 and P2 telemetry in real-time', async ({ page }) => {
    ...
    // Launch co-op mode and skip intro to active PLAYING state
    await page.evaluate(() => {
      const cheat = (window as any).__GALAGA_CHEAT__;
      const game = cheat.getGame();
      game.setCoopMode(true);
    ...
    await page.evaluate(() => {
      ...
      if (game.scoreManager) {
        game.scoreManager.setScore(1500, 'p1');
        game.scoreManager.setScore(3200, 'p2');
      }
      p1.score = 1500;
      p1.combo = 2;
      p1.specialEnergy = 75;
    ...
    await expect(p1Fill).toHaveAttribute('style', /width:\s*75%/);
    ...
    await expect(p2Fill).toHaveAttribute('style', /width:\s*100%/);
  ...
  test('TC-M35-COOP-04: Fatal hit triggers revive countdown alert and partner life donation revives player', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      p2.lives = 3;
      if (game.scoreManager) {
        game.scoreManager.setLives(1, 'p1');
        game.scoreManager.setLives(3, 'p2');
      }
      game.update(1 / 60);
    });
    await page.waitForTimeout(300);
